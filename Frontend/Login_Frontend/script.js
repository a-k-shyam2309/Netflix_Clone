...
function setupLogin() {
  ...
  loginForm.addEventListener(
    ...
    async event => {
      ...
      let userFullName = role === "admin" ? "Aditya Kumar Shyam" : "Aanya Sharma";
...
function setupRegisterForm() {
  ...
  const originalBtnText =
    submitBtn ? submitBtn.textContent : "Continue →";
  ...
  form.addEventListener(
    ...
    async event => {
      ...
      let userFullName = fullNameInput || "Aanya Sharma";

      if (window.CivicBuzzAPI && emailInput && password) {
        try {
          const regRole = (role || "citizen").toUpperCase();
          const res = await window.CivicBuzzAPI.auth.register({
            full_name: fullNameInput || "Civic Citizen",
            email: emailInput,
            password: password,
            role: regRole,
          });
